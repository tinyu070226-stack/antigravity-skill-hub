...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function createDiagramRealSvgCard(item) {
    ...
    const svgContainer = item.real_svg ? `
      <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 6px; margin-bottom: 14px; overflow: hidden; aspect-ratio: 16/9; display: flex; align-items: center; justify-content: center; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02); position: relative;">
  ...
  function createRichStyleMockupCard(item) {
    ...
    card.innerHTML = `
      ...
      <div class="trigger-group">
        <span class="trigger-text">${trigStr}</span>
        <button class="copy-btn" onclick="copyText(decodeURIComponent('${encodeURIComponent(currentLang === 'en' ? (item.copy_prompt_en || item.copy_prompt_zh || mainTrig) : (item.copy_prompt_zh || mainTrig))}'), '${currentLang === 'en' ? 'Prompt Copied' : '已複製提示詞'}', this)">${currentLang === 'en' ? 'Copy Prompt' : '複製指令'}</button>
      </div>
