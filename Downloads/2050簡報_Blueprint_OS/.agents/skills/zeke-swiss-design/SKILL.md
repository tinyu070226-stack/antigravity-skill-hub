...
A design system rooted in the Swiss International Style of the 1950s??0s: grotesque typography, rigorous grid, bold geometric forms, generous whitespace, and restrained color.
...
- Use the ellipsis character `?圳, never three periods `...`.
- Use curly quotes `"` `"` and `'` `'`, never straight quotes `"` `'`.
- Loading and progress states end with an ellipsis: `Loading?圳, `Saving?圳.
...
- Non-breaking spaces between value and unit, and inside brand names: `10&nbsp;MB`, `??nbsp;K`.
...
- **Headings are light, not bold.** `font-light` for display and h1, `font-normal` for h2?3.
...
- **Straight quotes and `...` are bugs.** Use curly quotes and the `?圳 character.
