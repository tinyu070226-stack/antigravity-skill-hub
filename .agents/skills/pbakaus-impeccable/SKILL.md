...
- **Verify contrast.** Body text must hit ??.5:1 against its background; large text (??8px or bold ??4px) needs ??:1. Placeholder text needs the same 4.5:1, not the muted-gray default. The most common failure: muted gray body text on a tinted near-white. If the contrast is even close, bump the body color toward the ink end of the ramp; light gray "for elegance" is the single biggest reason AI designs feel hard to read. <!-- rule:skill-color-verify-contrast -->
...
- Cap body line length at 65??5ch. <!-- rule:skill-typo-line-length -->
...
- Use `text-wrap: balance` on h1?3 for even line lengths; `text-wrap: pretty` on long prose to reduce orphans. <!-- rule:skill-typo-text-wrap-balance -->
...
- Tinted neutrals: add 0.005??.015 chroma toward the brand's hue. Don't default-tint toward warm or cool "because the brand feels that way"; that's the cross-project monoculture move. <!-- rule:skill-color-tinted-neutrals-chroma -->
...
- Pick a **color strategy** before picking colors. Four steps on the commitment axis: <!-- rule:skill-color-strategy-commitment -->
  - **Restrained**: tinted neutrals + one accent ??0%. Product default; brand minimalism.
  - **Committed**: one saturated color carries 30??0% of the surface. Brand default for identity-driven pages.
  - **Full palette**: 3?? named roles, each used deliberately. Brand campaigns; product data viz.
...
- **The hero-metric template.** Big number, small label, supporting stats, gradient accent. SaaS clich矇. <!-- rule:skill-ban-hero-metric -->
...
- **`border-radius: 32px+` on cards / sections / inputs.** You over-round. Cards top out at 12??6px; full-pill is fine for tags/buttons. Picking 24/28/32/40px on a card is the codex tell; no brand wants "insanely rounded". <!-- rule:skill-ban-codex-over-round -->
...
| `audit [target]` | Evaluate | Technical quality checks (a11y, perf, responsive) | [reference/audit.md](reference/audit.md) 繚 native: [reference/audit.native.md](reference/audit.native.md) |
...
| `adapt [target]` | Fix | Adapt for different devices and screen sizes | [reference/adapt.md](reference/adapt.md) 繚 native: [reference/adapt.native.md](reference/adapt.native.md) |
